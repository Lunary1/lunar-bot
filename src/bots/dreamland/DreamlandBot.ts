...
export class DreamlandBot extends StoreBot {
  private baseUrl = "https://www.dreamland.be";
  ...
  /**
   * Search for products on Dreamland
   */
  async searchProducts(
    ...
    try {
      ...
      await this.waitForElement(
        ".product-item, .product-card, .search-result-item",
        10000
      );
  ...
  async getProductDetails(
    ...
    try {
      ...
      await this.waitForElement(
        ".product-details, .product-info, .product-main",
        10000
      );
      ...
      const product = await this.page.evaluate(() => {
        ...
        const availabilityElement = document.querySelector(
          ".stock, .availability, .add-to-cart"
        );
  ...
  async addToCart(
    ...
    try {
      ...
      const isAvailable = await this.isElementVisible(
        ".add-to-cart, .add-to-cart-btn, .btn-add-to-cart"
      );
      if (!isAvailable) {
        return {
          success: false,
  ...
  async isLoggedIn(): Promise<boolean> {
    try {
      ...
      const loginIndicator = await this.page.$(
        ".user-menu, .account-menu, .logout-btn"
      );
