...
export class PokemonCenterBot extends StoreBot {
  private baseUrl = "https://www.pokemoncenter.com";
  ...
  /**
   * Search for products on Pokémon Center EU
   */
  async searchProducts(
    ...
    try {
      ...
      await this.waitForElement(
        ".product-tile, .product-item, .search-result-item",
        10000
      );
  ...
  async addToCart(
    ...
    try {
      ...
      await this.page.goto(`${this.baseUrl}/product/${productId}`);
      await this.randomDelay(2000, 4000);

      // Check if product is available
      const isAvailable = await this.isElementVisible(
        ".add-to-cart, .pdp-add-to-cart, .btn-add-to-cart"
      );
      if (!isAvailable) {
        return {
          success: false,
  ...
  /**
   * Login to Pokémon Center account
   */
